...
CONFIG="$ROOT/config/hypervisor/qemu-device-io.toml"
...
for command in "$QEMU" dd grep make mktemp printf truncate; do
    command -v "$command" >/dev/null 2>&1 || {
        echo "test-hv-device-io: missing command: $command" >&2
        exit 1
    }
done
...
make -C "$ROOT" hv-image \
    HV_CONFIG="$CONFIG" \
    HV_OUTPUT_IMAGE="$BOOT_IMAGE" \
    MNU_DIR="$MNU_DIR" \
    HOST_CARGO="${MBOOT_HOST_CARGO:-$(command -v cargo)}" \
    HOST_RUSTC="${MBOOT_HOST_RUSTC:-$(command -v rustc)}"
...
if [[ $verified -ne 1 ]]; then
    ...
    echo 'test-hv-device-io: virtio-blk DMA/IRQ verification failed' >&2
    exit 1
fi

grep -E 'IOMMU|PCI requester 0018|virtio-blk|Hardware Domain 2 ready' "$SERIAL"
echo 'test-hv-device-io: PASS'
