...
efi_cfg=$($ROOT/output/host/bin/mtype -i "$IMAGES/efi-part.vfat" ::/EFI/BOOT/grub.cfg)
printf '%s\n' "$efi_cfg" | grep -Fq "root=PARTUUID=$ROOT_PARTUUID" ||
	fail 'EFI GRUB configuration is incomplete'
...
for symbol in CONFIG_DRM_I915=y CONFIG_DRM_AMDGPU=y CONFIG_DRM_NOUVEAU=y \
	...
	CONFIG_KVM_INTEL=y CONFIG_KVM_AMD=y CONFIG_SATA_AHCI=y CONFIG_BLK_DEV_NVME=y \
