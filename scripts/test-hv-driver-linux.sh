...
for ((attempt = 0; attempt < TIMEOUT_SECONDS * 10; attempt++)); do
    if grep -Fq 'Driver Linux OK' "$WORK/serial.log" 2>/dev/null; then
        grep -E '\[mBoot-HV\]|Driver Linux OK|Linux version' "$WORK/serial.log"
        echo 'test-hv-driver-linux: PASS'
        exit 0
    fi
...
echo 'test-hv-driver-linux: Driver Linux did not reach init' >&2
exit 1
