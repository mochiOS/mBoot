...
for symbol in CONFIG_EFI CONFIG_ACPI CONFIG_PCI CONFIG_SATA_AHCI \
	...
	CONFIG_VFAT_FS CONFIG_EFIVAR_FS CONFIG_KVM CONFIG_VIRTIO_INPUT \
...
if grep -Eq -- 'virtio-(keyboard|mouse)-pci' \
...
grep -Fq '|| gui_fullscreen)' \
	board/mboot/patches/qemu/0001-sdl-keep-fullscreen-window-size.patch ||
	fail 'inner QEMU fullscreen resize patch is missing'
grep -Fq 'SDL_SetWindowInputFocus(scon->real_window)' \
	board/mboot/patches/qemu/0001-sdl-keep-fullscreen-window-size.patch ||
	fail 'inner QEMU fullscreen input focus patch is missing'
grep -Fq 'if (gui_grab || gui_fullscreen ||' \
	board/mboot/patches/qemu/0001-sdl-keep-fullscreen-window-size.patch ||
